...
[Fedora 43](https://docs.fedoraproject.org/en-US/releases/f43/) (Linux) for
integration testing.
...
- DevOps validation (like testing
  [Ansible collections](https://foundata.com/en/projects/ansible-collections/),
  roles, and playbooks)
...
**⭐ Found this useful? Support open-source and star this project:** [GitHub repository](https://github.com/foundata/oci-fedora43-itt)
<!-- rumdl-enable MD033 -->
...
- `1.0.0`, `1.0.1`, ...: A specific release, following
  ...
  always names the same image, see [`CHANGELOG.md`](https://github.com/foundata/oci-fedora43-itt/blob/main/CHANGELOG.md) for what
...
2. Pull the image from [Quay](https://quay.io/repository/foundata/fedora43-itt):
...
3. Run a container from the image:
   ...
   ```bash
   podman run --detach --name fedora43-itt quay.io/foundata/fedora43-itt:latest
   ```

   Note: **On SELinux-enabled systems**, `systemd` attempts to write to the
   cgroup filesystem, which might be denied by default security policies. To
   allow this operation, you must
   **enable the `container_manage_cgroup` boolean** on the host system:
   `sudo setsebool -P container_manage_cgroup 1`
4. Wait until `systemd` has finished booting inside the container:

   ```bash
   until state=$(podman exec fedora43-itt systemctl is-system-running 2>/dev/null); \
     [ "${state}" = running ] || [ "${state}" = degraded ]; do sleep 1; done
   ```

   `running` means every unit started. `degraded` means at least one unit
   failed, list them with `podman exec fedora43-itt systemctl --failed`. The
...
5. You can now work with the container, e.g. open a Bash terminal:
   ...
   ```bash
   podman exec -it fedora43-itt /bin/bash
   ```
...
[`DEVELOPMENT.md`](https://github.com/foundata/oci-fedora43-itt/blob/main/DEVELOPMENT.md) describes how to build the image locally,
...
published image anywhere. [`CHANGELOG.md`](https://github.com/foundata/oci-fedora43-itt/blob/main/CHANGELOG.md) documents what
changed between releases.


## Contributing<a id="contributing"></a>

See [`CONTRIBUTING.md`](https://github.com/foundata/oci-fedora43-itt/blob/main/CONTRIBUTING.md) for the contribution workflow and
[`DEVELOPMENT.md`](https://github.com/foundata/oci-fedora43-itt/blob/main/DEVELOPMENT.md) for local builds, tests and releases.
...
[`LICENSES/GPL-3.0-or-later.txt`](https://github.com/foundata/oci-fedora43-itt/blob/main/LICENSES/GPL-3.0-or-later.txt) for the full
text.

The [`REUSE.toml`](https://github.com/foundata/oci-fedora43-itt/blob/main/REUSE.toml) file provides detailed licensing and copyright
...
[REUSE status](https://api.reuse.software/info/github.com/foundata/oci-fedora43-itt)
