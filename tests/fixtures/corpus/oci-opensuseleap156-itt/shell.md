# OCI Image: openSUSE Leap 15.6, Integration Test Target (ITT)

[openSUSE Leap 15.6](https://doc.opensuse.org/release-notes/x86_64/openSUSE/Leap/15.6/)
(Linux) for integration testing.
...
[![GitHub repository](https://img.shields.io/github/stars/foundata/oci-opensuseleap156-itt.svg)](https://github.com/foundata/oci-opensuseleap156-itt)
...
- `1.0.0`, `1.0.1`, ...: A specific release, following
  ...
  always names the same image, see [`CHANGELOG.md`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/CHANGELOG.md) for what
...
2. Pull the image from
   [Quay](https://quay.io/repository/foundata/opensuseleap156-itt):

   ```bash
   podman pull quay.io/foundata/opensuseleap156-itt:latest
   ```

3. Run a container from the image:

   ```bash
   podman run --detach --name opensuseleap156-itt quay.io/foundata/opensuseleap156-itt:latest
   ```
...
4. Wait until `systemd` has finished booting inside the container:
   ...
   ```bash
   until state=$(podman exec opensuseleap156-itt systemctl is-system-running 2>/dev/null); \
     [ "${state}" = running ] || [ "${state}" = degraded ]; do sleep 1; done
   ```

   `running` means every unit started. `degraded` means at least one unit
   failed, list them with `podman exec opensuseleap156-itt systemctl --failed`.
...
5. You can now work with the container, e.g. open a Bash terminal:
   ...
   ```bash
   podman exec -it opensuseleap156-itt /bin/bash
   ```
...
[`DEVELOPMENT.md`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/DEVELOPMENT.md) describes how to build the image locally,
...
published image anywhere. [`CHANGELOG.md`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/CHANGELOG.md) documents what
changed between releases.


## Contributing<a id="contributing"></a>

See [`CONTRIBUTING.md`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/CONTRIBUTING.md) for the contribution workflow and
[`DEVELOPMENT.md`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/DEVELOPMENT.md) for local builds, tests and releases.
...
[`LICENSES/GPL-3.0-or-later.txt`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/LICENSES/GPL-3.0-or-later.txt) for the full
text.

The [`REUSE.toml`](https://github.com/foundata/oci-opensuseleap156-itt/blob/main/REUSE.toml) file provides detailed licensing and copyright
...
[![REUSE status](https://api.reuse.software/badge/github.com/foundata/oci-opensuseleap156-itt)](https://api.reuse.software/info/github.com/foundata/oci-opensuseleap156-itt)
...
- openSUSE® and SUSE® are registered trademarks of SUSE LLC or its affiliates
