# OCI Image: Ubuntu 26.04 LTS (Resolute Raccoon), Integration Test Target (ITT)
...
[![GitHub repository](https://img.shields.io/github/stars/foundata/oci-ubuntu2604-itt.svg)](https://github.com/foundata/oci-ubuntu2604-itt)
...
- `1.0.0`, `1.0.1`, ...: A specific release, following
  ...
  always names the same image, see [`CHANGELOG.md`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/CHANGELOG.md) for what
...
2. Pull the image from
   [Quay](https://quay.io/repository/foundata/ubuntu2604-itt):

   ```bash
   podman pull quay.io/foundata/ubuntu2604-itt:latest
   ```

3. Run a container from the image:

   ```bash
   podman run --detach --name ubuntu2604-itt quay.io/foundata/ubuntu2604-itt:latest
   ```
...
4. Wait until `systemd` has finished booting inside the container:
   ...
   ```bash
   until state=$(podman exec ubuntu2604-itt systemctl is-system-running 2>/dev/null); \
     [ "${state}" = running ] || [ "${state}" = degraded ]; do sleep 1; done
   ```

   `running` means every unit started. `degraded` means at least one unit
   failed, list them with `podman exec ubuntu2604-itt systemctl --failed`. The
...
5. You can now work with the container, e.g. open a Bash terminal:
   ...
   ```bash
   podman exec -it ubuntu2604-itt /bin/bash
   ```
...
[`DEVELOPMENT.md`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/DEVELOPMENT.md) describes how to build the image locally,
...
published image anywhere. [`CHANGELOG.md`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/CHANGELOG.md) documents what
changed between releases.


## Contributing<a id="contributing"></a>

See [`CONTRIBUTING.md`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/CONTRIBUTING.md) for the contribution workflow and
[`DEVELOPMENT.md`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/DEVELOPMENT.md) for local builds, tests and releases.
...
[`LICENSES/GPL-3.0-or-later.txt`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/LICENSES/GPL-3.0-or-later.txt) for the full
text.

The [`REUSE.toml`](https://github.com/foundata/oci-ubuntu2604-itt/blob/main/REUSE.toml) file provides detailed licensing and copyright
...
[![REUSE status](https://api.reuse.software/badge/github.com/foundata/oci-ubuntu2604-itt)](https://api.reuse.software/info/github.com/foundata/oci-ubuntu2604-itt)
