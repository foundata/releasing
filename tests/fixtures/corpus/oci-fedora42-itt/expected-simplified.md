...
[Fedora 42](https://docs.fedoraproject.org/en-US/releases/f42/) (Linux) for
integration testing.
...
**⭐ Found this useful? Support open-source and star this project:** [GitHub repository](https://github.com/foundata/oci-fedora42-itt)
<!-- rumdl-enable MD033 -->
...
- `1.0.0`, `1.0.1`, ...: A specific release, following
  ...
  always names the same image, see [`CHANGELOG.md`](https://github.com/foundata/oci-fedora42-itt/blob/main/CHANGELOG.md) for what
...
2. Pull the image from [Quay](https://quay.io/repository/foundata/fedora42-itt):
...
3. Run a container from the image:
   ...
   ```bash
   podman run --detach --name fedora42-itt quay.io/foundata/fedora42-itt:latest
   ```
...
4. Wait until `systemd` has finished booting inside the container:
   ...
   ```bash
   until state=$(podman exec fedora42-itt systemctl is-system-running 2>/dev/null); \
     [ "${state}" = running ] || [ "${state}" = degraded ]; do sleep 1; done
   ```

   `running` means every unit started. `degraded` means at least one unit
   failed, list them with `podman exec fedora42-itt systemctl --failed`. The
...
5. You can now work with the container, e.g. open a Bash terminal:
   ...
   ```bash
   podman exec -it fedora42-itt /bin/bash
   ```
...
[`DEVELOPMENT.md`](https://github.com/foundata/oci-fedora42-itt/blob/main/DEVELOPMENT.md) describes how to build the image locally,
...
published image anywhere. [`CHANGELOG.md`](https://github.com/foundata/oci-fedora42-itt/blob/main/CHANGELOG.md) documents what
changed between releases.


## Contributing<a id="contributing"></a>

See [`CONTRIBUTING.md`](https://github.com/foundata/oci-fedora42-itt/blob/main/CONTRIBUTING.md) for the contribution workflow and
[`DEVELOPMENT.md`](https://github.com/foundata/oci-fedora42-itt/blob/main/DEVELOPMENT.md) for local builds, tests and releases.
...
[`LICENSES/GPL-3.0-or-later.txt`](https://github.com/foundata/oci-fedora42-itt/blob/main/LICENSES/GPL-3.0-or-later.txt) for the full
text.

The [`REUSE.toml`](https://github.com/foundata/oci-fedora42-itt/blob/main/REUSE.toml) file provides detailed licensing and copyright
...
[REUSE status](https://api.reuse.software/info/github.com/foundata/oci-fedora42-itt)
